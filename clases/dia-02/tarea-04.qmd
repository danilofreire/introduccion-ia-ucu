...
*Pista:* `select_by_one_std_err()` toma los resultados del tuning, la métrica y la columna de penalty: `select_by_one_std_err(resultados_lasso, metric = "rmse", desc(penalty))`.
...
*Pista:* Random Forest para regresión usa la misma `rand_forest()` que en el Laboratorio 3, pero con `set_mode("regression")`: `rand_forest(trees = 500, mtry = tune(), min_n = tune()) |> set_engine("ranger") |> set_mode("regression")`.
