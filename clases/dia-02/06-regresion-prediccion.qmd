...
- Regularización impone un [presupuesto]{.alert}: el modelo reparte un total finito entre todas las variables
- $\lambda$ bajo: presupuesto generoso, muchas variables (similar a OLS)
- $\lambda$ alto: presupuesto ajustado, solo las variables que más ayudan
:::
:::
:::
...
receta <- recipe(voto ~ ., data = datos_train) |>
  ...
  step_interact(terms = ~ edad:educacion_anios) |>  # interacción
  step_impute_median(all_numeric_predictors()) |>   # imputar numéricas
  step_other(religion, threshold = 0.05) |>         # agrupar raras
  step_corr(all_numeric_predictors(),
            threshold = 0.9) |>                     # quitar r > 0.9 (multicolinealidad)
  step_log(ingreso, offset = 1) |>
  step_poly(edad, degree = 2)                       # edad + edad²
```
:::
...
modelo_lasso_tune <- linear_reg(penalty = tune(), mixture = 1) |> set_engine("glmnet")

# 2. Grilla de valores candidatos de penalty (λ) en escala log10
grilla_lambda <- grid_regular(penalty(range = c(-4, 0)), levels = 30)

# 3. Validación cruzada con 5 folds
folds <- vfold_cv(datos_train, v = 5)

# 4. Probar cada λ con CV
resultados <- tune_grid(modelo_lasso_tune,
  satisfaccion_vida ~ .,
  resamples = folds,
  grid = grilla_lambda,
  metrics = metric_set(rmse, rsq))

# 5. Visualizar y seleccionar el mejor
autoplot(resultados)
...
- Siempre comparar varios modelos con CV
:::
:::

<br>

[En ciencias sociales, la elección depende del objetivo: ¿explicar o predecir?]{.alert} Comparar varios modelos con validación cruzada es la mejor forma de elegir.
:::
