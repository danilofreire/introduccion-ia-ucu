...
Los coeficientes están en escala normalizada (aplicamos `step_normalize()`), así que cada uno mide el efecto de una desviación estándar del predictor. Ordenados por |z|, el efecto más fuerte es el de `educacion_anios` (z ≈ -3.3; odds ratio ≈ 0.65), seguido por `interes_politica` y `edad`. Los tres tienen coeficiente negativo: más educación, más interés político y más edad se asocian con *mayor* probabilidad de votar, justo lo que esperábamos teóricamente.

¿Cuál tiene un efecto negativo sobre el voto? `percepcion_economia` tiene coeficiente positivo (≈ +0.06, odds ratio ≈ 1.07), lo que significa que quienes ven mejor la economía votan algo *menos*: es el patrón de voto de protesta. Eso sí, su z es pequeño (≈ 0.6), así que en esta muestra el efecto no es estadísticamente significativo y conviene ser cauteloso.
...
**Respuesta:** Los hiperparámetros seleccionados cambian un poco (en nuestra corrida `mtry` se mantiene pero `min_n` pasa de 13 a 21), lo cual es normal: con remuestreos distintos, varias combinaciones rinden casi igual y la "mejor" puede variar. Lo interesante es el error estándar: no baja con más folds. En nuestra corrida pasa de ≈ 0.020 con 10 folds a ≈ 0.028 con 20, es decir, sube un poco. La razón es que con 20 folds cada conjunto de validación tiene apenas unas 19 observaciones, así que el AUC de cada fold es más ruidoso; promediar sobre más particiones no compensa esa mayor variabilidad. El beneficio real de usar más folds es reducir el sesgo de la estimación (cada modelo entrena con más datos), no necesariamente la varianza. Por eso 10 folds es el compromiso habitual entre estabilidad y tiempo de cómputo.
