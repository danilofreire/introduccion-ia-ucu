...
:::{.column width=50%}
:::{style="text-align: center;"}
[![](figures/supervised-learning.png){width="90%"}](#){data-modal-type="image" data-modal-url="figures/supervised-learning.png"}
:::
:::
:::
:::
...
[Interpretación:]{.alert} Por cada año adicional de educación, la probabilidad de apoyar la democracia aumenta un 8%.
:::
:::
:::
:::
...
- El gráfico muestra:
    - Las [reglas de decisión]{.alert} en cada nodo
    - La [proporción de clases]{.alert} en cada hoja
    - El [camino]{.alert} que sigue cada observación
:::
...
- Analogía: si les piden a 500 personas que estimen el peso de un buey, [el promedio será sorprendentemente cercano al valor real]{.alert}. Francis Galton documentó este fenómeno en 1907; Surowiecki lo popularizó como "la sabiduría de la multitud"
...
- Disponible en R con el paquete `shapr` o `DALEX`
...
- Las barras muestran la [importancia relativa]{.alert}
- Variables arriba = más importantes
- Pero [no indica la dirección]{.alert} del efecto
- Para eso necesitamos gráficos de dependencia parcial (PDP)
:::
:::
...
# PDP para edad
pdp_edad <- model_profile(explainer,
                          variables = "edad")
plot(pdp_edad)
```
:::
...
- Aplicable a América Latina donde los censos son menos frecuentes
:::

:::{.column width=50%}
**Aplicaciones regionales:**

- [Predicción de deserción escolar]{.alert} (Chile, Colombia): identificar estudiantes en riesgo antes de que abandonen
- [Scoring crediticio alternativo]{.alert} (México, Brasil): Nubank y otras fintech usan ML para evaluar clientes sin historial bancario
- [Detección de fraude electoral]{.alert}: análisis de patrones anómalos en resultados
- [Predicción de violencia]{.alert}: modelos para anticipar zonas de riesgo (Colombia, México)
- [Análisis de opinión pública]{.alert}: clasificación de tweets y comentarios sobre políticas
...
## Ejemplo: Deserción escolar en Chile
...
- El Ministerio de Educación de Chile quiere identificar estudiantes en riesgo de abandonar la escuela
- Datos disponibles: asistencia, notas, información socioeconómica, historia escolar
- Objetivo: intervenir [antes]{.alert} de que abandonen

**Enfoque con Random Forest:**

1. Entrenar modelo con datos históricos (estudiantes que abandonaron vs. los que no)
2. Predecir probabilidad de abandono para estudiantes actuales
3. Priorizar intervenciones en los de mayor riesgo
4. Usar importancia de variables para entender factores de riesgo
:::
...
─────────────────────────────────
dias_ausencia_mes     ████████ 0.15
promedio_notas        ██████   0.11
repitencia_previa     █████    0.09
ingreso_hogar         ████     0.07
distancia_escuela     ███      0.05
educacion_madre       ██       0.04
...
```

[La ausencia escolar es el mejor predictor]{.alert}: los estudiantes empiezan a faltar antes de abandonar definitivamente.

Esto sugiere intervenciones tempranas cuando aumenta el ausentismo.
:::
:::
:::
:::
