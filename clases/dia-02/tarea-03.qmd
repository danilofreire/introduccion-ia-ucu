...
*Pista:* presten atención a cómo definimos los niveles de `voto` en la configuración (`c("si", "no")`). R modela la probabilidad del segundo nivel, así que verifiquen en qué dirección apunta cada coeficiente antes de interpretarlo: un signo puede significar lo contrario de lo que parece a primera vista.
...
*Pista (métricas):* para cada modelo necesitan tanto la clase predicha como la probabilidad; `augment()` les da ambas en un paso. Después combinen `metrics()` con `f_meas()` (y `precision()`/`recall()` si las quieren) para armar la tabla.
