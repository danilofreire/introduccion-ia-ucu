...
- [Interpretable]{.alert}: "por cada año adicional de educación, las chances de votar aumentan un X%"
:::
...
3. [Dividir los datos en dos grupos]{.alert} según esa regla
4. [Repetir]{.alert} recursivamente para cada grupo
5. [Parar cuando se cumple un criterio:]{.alert}
    - Profundidad máxima
...
```{r poda-tradeoff, echo=TRUE, message=FALSE, warning=FALSE, fig.height=3.5, fig.width=5, fig.align="center", out.width="100%"}
#| code-fold: true
...
# Gradient Boosting {background-color="#2d4563"}
...
:::{.columns}
:::{.column width=55%}
- Random Forest entrena muchos árboles [en paralelo]{.alert} y promedia
- [Gradient Boosting]{.alert} los entrena [en secuencia]{.alert}: cada árbol nuevo intenta corregir los errores del anterior
- Suele dar [muy buen rendimiento]{.alert}, pero tiene [muchos hiperparámetros]{.alert} (número de árboles, profundidad, tasa de aprendizaje), así que el tuning que acabamos de ver [se vuelve esencial]{.alert}
...
# Los cuatro ajustes de esta sesión (misma estructura, distinta spec)
...
ajuste_rf <- rand_forest(trees = 2000, mtry = 2, min_n = 5) |> set_engine("ranger", seed = 1, importance = "permutation") |> set_mode("classification") |>
  fit(apoya_democracia ~ ., data = datos_train)
ajuste_xgb <- boost_tree(trees = 500, tree_depth = 3, learn_rate = 0.05, min_n = 10) |> set_engine("xgboost", seed = 1) |> set_mode("classification") |>
  fit(apoya_democracia ~ ., data = datos_train)
...
- Acá [gana la regresión logística]{.alert}, y ni el Random Forest ni el XGBoost la superan. No es casualidad: [estos datos se generaron con relaciones lineales]{.alert}, y ningún modelo complejo puede encontrar una estructura no lineal que no existe
- [Más complejo no es sinónimo de mejor]{.alert}. Con datos reales, más ruidosos y no lineales, el orden suele invertirse. Por eso [siempre hay que comparar]{.alert} antes de elegir
:::
...
|----------|---------------|-------|---------------|---------|
| [Interpretabilidad]{.alert} | Alta | Alta (visual) | Baja | Baja |
| [Capacidad predictiva]{.alert} | Moderada | Moderada-baja | Alta | Alta |
| [Riesgo de sobreajuste]{.alert} | Bajo | Alto | Bajo | Medio-alto |
| [Velocidad]{.alert} | Muy rápida | Rápida | Moderada | Moderada |
| [Relaciones no lineales]{.alert} | No | Sí | Sí | Sí |
| [Hiperparámetros]{.alert} | Ninguno | Pocos | Pocos | Muchos |
...
- Si necesitan la [mejor predicción posible]{.alert}: Random Forest o Gradient Boosting, pero solo si los datos tienen estructura no lineal que aprovechar
...
- Cuatro modelos, del más interpretable al más predictivo: [regresión logística]{.alert}, [árboles de decisión]{.alert}, [Random Forests]{.alert} y [Gradient Boosting]{.alert}
...
- El modelo más complejo [no siempre gana]{.alert}: cuando la relación es lineal, la regresión logística es difícil de superar

**Para explorar:**

- [LightGBM]{.alert} y [CatBoost]{.alert}, otras variantes de boosting, y [SHAP]{.alert} para interpretar modelos de caja negra

:::{style="margin-top: 40px; text-align: center; font-size: 24px;"}
