...
- [`augment()`]{.alert} agrega las predicciones (`.pred_class`, `.pred_no`, `.pred_si`) al test set, así una sola llamada nos da todo lo necesario para calcular cualquier métrica de yardstick
:::
...
ajuste_rf <- rand_forest(trees = 500, mtry = 3, min_n = 5) |>
...
# Reusamos ajuste_rf,
# que ya tiene importance = "permutation"
...
modelo_rf_tune <- rand_forest(trees = 500, mtry = tune(), min_n = tune()) |>
...
pred_test <- augment(ajuste_final, datos_test)
pred_test |> accuracy(truth = apoya_democracia, estimate = .pred_class)
pred_test |> roc_auc(truth = apoya_democracia, .pred_si, event_level = "second")
```
...
# Recap: las tres ajustes que vimos en este capítulo (misma estructura, distinta spec)
...
ajuste_arbol <- decision_tree(tree_depth = 5, min_n = 10) |> set_engine("rpart") |> set_mode("classification") |>
  fit(apoya_democracia ~ ., data = datos_train)
ajuste_rf <- rand_forest(trees = 500, mtry = 3, min_n = 5) |> set_engine("ranger", seed = 1, importance = "permutation") |> set_mode("classification") |>
  fit(apoya_democracia ~ ., data = datos_train)

# Métrica: accuracy en test usando augment() + accuracy() para cada modelo
tibble(
  modelo = c("Logística", "Árbol", "Random Forest"),
  accuracy = c(
    augment(ajuste_log,   datos_test) |> accuracy(apoya_democracia, .pred_class) |> pull(.estimate),
    augment(ajuste_arbol, datos_test) |> accuracy(apoya_democracia, .pred_class) |> pull(.estimate),
    augment(ajuste_rf,    datos_test) |> accuracy(apoya_democracia, .pred_class) |> pull(.estimate)
  )
