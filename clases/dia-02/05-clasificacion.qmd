...
- No captura [interacciones complejas]{.alert} automáticamente
- Puede ser insuficiente con patrones no lineales
:::
...
:::{.columns}
:::{.column width=55%}
**Pregunta de investigación:** ¿qué factores predicen si un uruguayo apoya la democracia? Datos del Latinobarómetro Uruguay (simulados)

**Variables:** edad, educación, ingreso, confianza en instituciones, percepción económica, zona, género

**Outcome:** `apoya_democracia` (sí/no)
...
- Más árboles mejoran (con rendimientos decrecientes)
- `mtry` = $\sqrt{p}$ para clasificación, $p/3$ para regresión. Restringir variables por split [fuerza a los árboles a ser diferentes]{.alert}
- En la práctica, [los valores por defecto funcionan bien]{.alert}
:::
:::
:::
:::
...
:::{.columns}
:::{.column width=55%}
**Importancia por permutación** (la que usamos)
...
- Se activa con `importance = "permutation"` en el engine
:::

:::{.column width=45%}
**Otras opciones**

- [Gini (impureza)]{.alert}: suma de reducciones de impureza. Rápida, pero sesgada hacia variables con muchas categorías
- [SHAP values]{.alert}: contribución de cada variable a cada predicción individual. Más avanzado; disponible con `shapr` o `fastshap`

<br>

[Para este curso usamos importancia por permutación, la más robusta]{.alert}
:::
:::
:::
...
## Ajuste de hiperparámetros con tidymodels

:::{style="margin-top: 30px; font-size: 20px;"}

El [tuning]{.alert} busca la mejor combinación de hiperparámetros usando validación cruzada (nunca el conjunto de prueba):

```{r tuning-full, echo=TRUE, message=FALSE, warning=FALSE}
# 1. Modelo con parámetros tune()
...
# 2. Grilla de valores + folds para validación cruzada
grilla <- grid_regular(mtry(range = c(2, 6)),
                       min_n(range = c(5, 20)),
                       levels = 4)
folds  <- vfold_cv(datos_train, v = 5, strata = apoya_democracia)

# 3. Buscar la mejor combinación (puede tardar ~1-2 min la primera vez)
set.seed(1)
...
- `tune()` marca los hiperparámetros a ajustar; `tune_grid()` evalúa cada combinación con validación cruzada
- `select_best()` elige la mejor según la métrica indicada; `finalize_model()` fija los hiperparámetros para el entrenamiento final
- [El tuning mejora el rendimiento pero toma tiempo]{.alert} — empiecen con valores por defecto y ajusten si es necesario
:::
