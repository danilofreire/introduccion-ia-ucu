...
- Paquetes principales: [tidytext]{.alert} (tokenización y TF-IDF), [tm]{.alert} (stopwords), [topicmodels]{.alert} (LDA), además de tidyverse
:::
...
- Aun así, [funciona razonablemente bien]{.alert} a gran escala
:::
...
- Las [palabras más probables]{.alert} de cada tema
...
:::{.columns}
:::{.column width=55%}
- Bag-of-words y TF-IDF son [representaciones dispersas]{.alert} (sparse): cada palabra es una dimensión independiente y no capturan [significado]{.alert}
    - "perro" y "can" son tan distintos como "perro" y "democracia"
    - No manejan [sinónimos]{.alert}, [polisemia]{.alert} ni [orden]{.alert}
- Un [embedding]{.alert} es un vector [denso]{.alert} que representa una palabra: las que tienen significados similares quedan [cercanas]{.alert}
- Idea central: "una palabra se conoce por la compañía que frecuenta" (Firth, 1957). Si dos palabras aparecen en [contextos similares]{.alert}, sus embeddings se parecen
- Métodos clásicos: [Word2Vec]{.alert} (2013), GloVe (2014); modernos: [BERT]{.alert}, GPT (mañana)
:::
...
- Entrenarlos requiere [muchísimos datos]{.alert} (miles de millones de palabras), así que en la práctica usamos [preentrenados]{.alert}:
    - [Word2Vec](https://code.google.com/archive/p/word2vec/) (Google News, inglés)
    - [fastText](https://fasttext.cc/) (Facebook): hay versión para [español](https://fasttext.cc/docs/en/crawl-vectors.html)
...
- Para representar [documentos completos]{.alert}:
    - [Promedio]{.alert} de los embeddings de sus palabras (con o sin ponderar por TF-IDF)
    - [Doc2Vec]{.alert}: extensión de Word2Vec
    - [Sentence-BERT]{.alert}: embeddings de oraciones con transformers
- Permiten [medir similitud semántica]{.alert} entre textos
:::
:::

:::{style="text-align: center; margin-top: 12px; font-size: 19px;"}
En el apéndice construimos mini-embeddings con LSA (SVD sobre TF-IDF) y buscamos palabras y documentos similares en nuestro corpus &nbsp; [[Ver demo (LSA)]{.button}](#sec:apendice-lsa)
:::
:::

# Aplicaciones {background-color="#2d4563"}

## Aplicaciones en política y medios {#sec:resumen-3-2}
...
:::{style="text-align: center; margin-top: 8px; font-size: 19px;"}
[[Ver caso integrado completo]{.button}](#sec:apendice-caso)
:::
:::
...
| Descubrir [temas latentes]{.alert} sin etiquetas | [LDA]{.alert} | No supervisado, útil si no saben qué temas buscan |
| Medir el [tono]{.alert} de un texto | [Diccionarios]{.alert} de sentimiento | Transparente y validable |
| Medir [similitud semántica]{.alert} entre textos | [Embeddings]{.alert} (Word2Vec, LSA) | Capturan significado, no solo palabras |
...
Regla práctica: [empezar simple]{.alert}, validar contra lectura manual y escalar a métodos complejos solo si el simple falla
:::
:::
...
- [Caso integrado]{.alert} sobre encuesta abierta (apéndice)
...
## Mini-embeddings de palabras con LSA {#sec:apendice-lsa}

:::{style="margin-top: 20px; font-size: 19px;"}

Aplicamos [SVD]{.alert} sobre la matriz TF-IDF (palabra x documento) para obtener mini-embeddings: cada palabra y cada documento queda representado en 10 dimensiones. Es la idea detrás de [LSA]{.alert} (Latent Semantic Analysis)
...
sims <- apply(embeddings, 1, coseno, b = objetivo)
...
Los documentos más cercanos al primero (tema [economía]{.alert}) son otros del mismo tema y uno de medio ambiente, que comparte vocabulario económico. LSA aprende la similitud sin conocer las etiquetas
...
## Caso integrado (1/2): explorar las respuestas {#sec:apendice-caso}
