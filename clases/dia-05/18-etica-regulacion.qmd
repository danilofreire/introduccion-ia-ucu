...
- Sesgos [inconsistentes]{.alert}: dependen de quién lea su carpeta, de su humor, de la hora
...
1. Definir los [grupos protegidos]{.alert} (género, etnicidad, edad...). A veces solo hay proxies
...
- [Inferencia]{.alert}: el modelo deduce lo que nunca contaron. Los "me gusta" predicen orientación política; las compras, un embarazo; el celular, síntomas de depresión
...
:::{style="margin-top: 15px; background: rgba(45, 69, 99, 0.1); padding: 15px; border-radius: 10px;"}
El problema no es solo qué datos entregamos, sino [qué se puede inferir]{.alert} de ellos. El consentimiento que firmamos rara vez cubre eso.
:::
:::
:::
:::
...
- Y muestra el límite de la autorregulación: la empresa solo cambió cuando llegaron [demandas y multas]{.alert}
...
- Principios: [minimización]{.alert} (recolectar solo lo necesario), limitación de propósito, plazos de conservación
...
- [Reportar]{.alert} métricas por subgrupos, no solo el promedio
...
[Si no están en los datos, el modelo no tiene nada que aprender sobre ustedes.]{.alert}
:::
...
- ¿El algoritmo debería [corregir]{.alert} la injusticia histórica, o solo [no perpetuarla]{.alert}?
