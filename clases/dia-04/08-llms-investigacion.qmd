...
```{r lab-llm-setup, eval=FALSE, prompt=FALSE}
# Instalar ellmer
...
# IMPORTANTE: creamos un chat nuevo cada vez para evitar que el
# historial de conversación acumule contexto y afecte las respuestas
...
resultados <- textos |>
  slice(1:20) |>
  mutate(
    tema_llm = map_chr(texto, clasificar_texto)
  )
...
- Los [LLMs]{.alert} predicen el siguiente token, pero a gran escala muestran capacidades que no fueron programadas explícitamente
