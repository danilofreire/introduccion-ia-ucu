...
Word2Vec ya hacía algo notable: [aprender que palabras similares tienen vectores cercanos]{.alert}, sin etiquetas, solo prediciendo contexto
...
# La única idea: predicción del próximo token {background-color="#2d4563"}
...
Solo [estadísticas sobre tokens]{.alert}, computadas con una arquitectura muy potente, sobre datos masivos
...
- Vocabulario [finito]{.alert} (típicamente 50.000 a 200.000 tokens) que puede representar casi [cualquier texto]{.alert}
...
Para saber [qué estaba caliente]{.alert}, el modelo computa atención desde "estaba":
...
- El modelo "resuelve" que lo caliente era la [alfombra]{.alert} y no la [gata]{.alert}, porque la atención lo pondera así
:::
...
- Los transformers usan [atención multi-cabeza]{.alert}: varios mecanismos de atención en paralelo dentro de la misma capa
...
- Solo las grandes empresas (OpenAI, Google, Anthropic, Meta, xAI) pueden entrenar modelos de frontera. [Nosotros usamos los modelos pre-entrenados]{.alert}
:::
:::
...
- Esto se llama [emergencia]{.alert} y es debatido: ¿es real o artefacto de medición? ([Schaeffer et al., 2023](https://arxiv.org/abs/2304.15004))
:::
...
[Sin entrenamiento, sin actualizar pesos.]{.alert} Solo le pasamos los ejemplos en el contexto. El modelo continúa el patrón:
...
- El modelo no tiene acceso a [hechos]{.alert}, solo a [patrones estadísticos del texto de entrenamiento]{.alert}
...
:::{.column width=45%}
:::{style="text-align: center;"}
[![Tasas de alucinación de los 25 modelos con mejor desempeño](figures/hallucination-rates-vectara.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/hallucination-rates-vectara.png"}

:::{style="font-size: 18px; color: #555;"}
Fuente: [Vectara Hallucination Leaderboard](https://github.com/vectara/hallucination-leaderboard) (julio de 2026)
:::

:::{style="font-size: 20px; margin-top: 15px;"}
El modelo resume un documento que le dan, y se cuenta cuántas veces [agrega datos que no están en ese documento]{.alert}. Aun con la fuente a la vista, del 2% al 7% de los resúmenes inventan algo
:::
:::
:::
:::
...
- [Cons]{.alert}: cuestan dinero, requieren tarjeta de crédito, son [cajas negras]{.alert} (no pueden inspeccionarlos), pueden cambiar entre versiones
...
- Buena herramienta: [Ollama]{.alert} (`ollama pull granite4.1:3b` y listo)
...
- [MoE]{.alert}: activan solo parte de sus parámetros por token
