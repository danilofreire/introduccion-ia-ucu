...
- Construido sobre [ellmer](https://ellmer.github.io/): usa los mismos proveedores (OpenRouter, Ollama, etc.)
...
El `schema` usa los tipos de `ellmer` (Lab 7). `type_integer()` pide un número entero; `levels` le dice a `quallmer` que `score` es una variable de [intervalo]{.alert}, para calcular las métricas correctas
:::
...
:::{.callout-warning}
Con `max_active` alto, los modelos `:free` devuelven errores `400`. Bajalo a 2-3, usá muestras chicas, o pasá a un modelo de pago. Para corpus grandes conviene un modelo local (Ollama, día 5)
:::
:::
...
- Guarda en un `.rds` todo lo necesario para que otra persona [replique]{.alert} tu codificación
:::

:::{.column width=45%}
- Es la [defensa 4]{.alert} de la lecture 14 (validación y reproducibilidad), automatizada
...
[Mismo cargo, sólo cambia el nombre. ¿Cambian las palabras que el modelo usa según el género?]{.alert}
:::
...
[Convertimos el texto en una medida comparable: ahora el sesgo es un número, no una impresión. Con tres nombres por género es sólo una demostración; para un estudio real harían falta más nombres y otras dimensiones (por ejemplo, competencia vs calidez)]{.alert}
:::
...
- Modelo exacto y versión (`nvidia/nemotron-3-super-120b`, fecha de uso)
...
- Cómo funcionan los LLMs (lecture 13)
- Las cuatro defensas para usarlos en investigación (lecture 14)
...
**Tarea (8 min):** ampliá el codebook para que, además del `score`, identifique la `dimension` retórica dominante (nacionalismo / seguridad / derechos / pluralismo / economia). Recodificá los discursos y mostrá una tabla cruzada de `dimension` por signo del `score`
...
**Tarea (8 min):** usá un LLM como [clasificador]{.alert}. Tenés un dataset con frases de dos partidos ficticios. Definí un codebook que puntúe la orientación económica de cada frase (de -5 estatista a +5 promercado), codificá las frases y compará el puntaje medio por partido. ¿El modelo separa a los dos partidos?
...
3. Codificar a mano una muestra de 15-20 textos (tu gold standard)
4. Codificar los mismos textos con `qlm_code()`
5. Validar contra tu codificación humana con `qlm_validate()`
