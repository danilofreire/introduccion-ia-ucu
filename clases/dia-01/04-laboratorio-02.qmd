...
- [Inflación alta]{.alert}: tasa mayor a 5% (`inflacion`)
...
Con 179 países y 5 folds, cada partición tiene ~27 países en validación. Es suficiente para entrenar, pero las métricas tendrán [más ruido]{.alert}
:::
...
# Acá pongan su feature en lugar del de la demo (ejemplo con internet_grupos)
...
eval_log_est <- fit_resamples(
  modelo_logistico, formula_estudiante, resamples = folds,
  metrics = metric_set(precision, recall, roc_auc),
  control = control_resamples(event_level = "second")
) |>
...
eval_arb_est <- fit_resamples(
  modelo_arbol, formula_estudiante, resamples = folds,
  metrics = metric_set(precision, recall, roc_auc),
  control = control_resamples(event_level = "second")
) |>
...
# Fórmula con el feature nuevo (internet_grupos)
