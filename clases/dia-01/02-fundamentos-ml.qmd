...
- [Data leakage:]{.alert} información del futuro que "se filtra" al entrenamiento
:::
...
- Para datos con [estructura temporal]{.alert} (series de tiempo)
...
- Reglas prácticas (muy aproximadas):
    - [Mínimo:]{.alert} 10 observaciones por variable predictora
    - [Clasificación:]{.alert} al menos 50-100 por clase minoritaria
    - [Deep learning:]{.alert} miles a millones de ejemplos
:::
...
Con más datos, el rendimiento mejora, pero con [rendimientos decrecientes]{.alert}.
...
:::{.columns}
:::{.column width=55%}
- [Regularización:]{.alert} técnica para penalizar la complejidad del modelo
...
- [L1 (LASSO):]{.alert} puede llevar coeficientes a exactamente cero (selección de variables)
- [L2 (Ridge):]{.alert} reduce coeficientes pero no los elimina
- [Elastic Net:]{.alert} combinación de L1 y L2
...
- [Early stopping]{.alert} (detener el entrenamiento antes de sobreajustar)
...
- [Media armónica]{.alert} de precisión y recall
- F1 = 2 × (precision × recall) / (precision + recall)
- Un solo número que [balancea ambas métricas]{.alert}
- F1 = 1 es perfecto; F1 = 0 es el peor caso
- Útil cuando [no podés priorizar]{.alert} una métrica sobre la otra

**Macro vs. Micro F1:**

- [Macro F1:]{.alert} promedio simple de F1 por clase (trata todas las clases igual)
- [Micro F1:]{.alert} calcula F1 global (favorece clases mayoritarias)
:::

:::{.column width=50%}
**AUC-ROC**

- [Curva ROC:]{.alert} grafica tasa de verdaderos positivos vs. falsos positivos
- [AUC:]{.alert} área bajo la curva ROC (0.5 = azar, 1.0 = perfecto)
...
# Hiperparámetros y selección de modelos {background-color="#2d4563"}
...
- [Parámetros:]{.alert} los aprende el modelo durante el entrenamiento (coeficientes, pesos)
- [Hiperparámetros:]{.alert} los definimos nosotros antes de entrenar

**Ejemplos de hiperparámetros:**

| Modelo | Hiperparámetros |
|--------|-----------------|
| Regresión regularizada | λ (fuerza de regularización) |
| Árbol de decisión | Profundidad máxima, mínimo de observaciones por hoja |
| Random Forest | Número de árboles, variables por split |
| K-NN | Número de vecinos (k) |
| Red neuronal | Tasa de aprendizaje, capas, neuronas |

[Elegir bien los hiperparámetros puede mejorar mucho el rendimiento.]{.alert}
:::
...
Para cada combinación de hiperparámetros:
  ...
  3. Guardar rendimiento

Elegir la combinación con mejor rendimiento
...
- [Grid search:]{.alert} probar todas las combinaciones
- [Random search:]{.alert} probar combinaciones aleatorias
- [Bayesian optimization:]{.alert} búsqueda inteligente

<br>

[Nunca ajustar hiperparámetros usando los datos de test.]{.alert}
:::
:::
:::
:::
...
```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│  Definir    │───▶│  Recoger    │───▶│  Entrenar   │───▶│  Desplegar  │───▶│  Monitorear │
│  problema   │    │  datos      │    │  y evaluar  │    │  (deploy)   │    │  y mantener │
└─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘
                                                                                    │
                                                                                    ▼
                                                                            ┌─────────────┐
...
:::{.columns}
:::{.column width=50%}
- El entrenamiento es [solo una parte]{.alert} del ciclo
- Después de desplegar, el modelo puede [degradarse]{.alert} (model drift)
- Los datos del mundo real [cambian con el tiempo]{.alert}
- Hay que [monitorear]{.alert} el rendimiento en producción
:::

:::{.column width=50%}
- Causas de degradación:
    - [Data drift:]{.alert} la distribución de los datos cambia
    - [Concept drift:]{.alert} la relación entre variables cambia
    - [Cambios en el negocio:]{.alert} nuevos productos, mercados
- [Un modelo no es "listo y olvidado"]{.alert}
:::
:::
:::
...
- [renv:]{.alert} gestión de dependencias (versiones de paquetes)
- [targets:]{.alert} pipelines reproducibles
- [Quarto/RMarkdown:]{.alert} código + documentación juntos
- [Git:]{.alert} control de versiones

**En general:**

- [Docker:]{.alert} contenedores con ambiente completo
- [MLflow:]{.alert} tracking de experimentos
- [DVC:]{.alert} versionado de datos
:::
...
- La [accuracy no es suficiente]{.alert}: necesitamos precisión, recall, F1, AUC-ROC
