...
- Convertir [variables categóricas]{.alert} a numéricas con [one-hot encoding](https://en.wikipedia.org/wiki/One-hot) o [label encoding](https://en.wikipedia.org/wiki/Feature_engineering#Categorical_data)
...
- Cada fold mantiene la [misma proporción de clases]{.alert} que el dataset
- Útil cuando las clases están [desbalanceadas]{.alert}
...
- [Nunca entrenar con el futuro]{.alert} para predecir el pasado

**[Leave-One-Out](https://en.wikipedia.org/wiki/Cross-validation_(statistics)#Leave-one-out_cross-validation) (LOO)**

- K = N, una observación por fold
- Útil con [muy pocos datos]{.alert}, pero costoso
:::
...
- En la práctica:
    ...
    - La [regularización]{.alert} controla la varianza sin aumentar mucho el sesgo
:::
...
- [[L2 (Ridge)](https://en.wikipedia.org/wiki/Ridge_regression)]{.alert} reduce los coeficientes, pero no los elimina
...
- La [curva ROC](https://es.wikipedia.org/wiki/Curva_ROC){.alert} grafica tasa de verdaderos positivos vs. falsos positivos
- El [AUC](https://en.wikipedia.org/wiki/Receiver_operating_characteristic#AUC){.alert} es el área bajo esa curva (0.5 = azar, 1.0 = perfecto)
- Mide qué tan bien el modelo [separa las clases]{.alert}
- [Independiente del umbral]{.alert}, ya que evalúa todas las probabilidades
...
- [Quarto/RMarkdown]{.alert}, código y documentación juntos
- [Git]{.alert} para control de versiones
