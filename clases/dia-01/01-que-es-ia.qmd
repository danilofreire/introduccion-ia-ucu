...
| | Supervisado | No supervisado | Por refuerzo |
|---|---|---|---|
| [Datos]{.alert} | Etiquetados | Sin etiquetar | Entorno interactivo |
...
| [Analogía]{.alert} | Con solucionario | Explorar sin mapa | Andar en bici |
:::

:::{style="margin-top: 30px; font-size: 28px;"}
[En la práctica, muchos sistemas combinan varios tipos.]{.alert} ChatGPT usa pre-entrenamiento auto-supervisado, ajuste con instrucciones (supervisado) y RLHF (refuerzo).
:::
...
- [Un modelo con 95% de accuracy siempre es mejor que uno con 80%]{.alert}
    - [Falso.]{.alert} Si el 95% de los casos son de una clase, predecir siempre esa clase da 95% sin aprender nada. Las métricas dependen del contexto y del balance de clases

- [Los LLMs "entienden" el significado de las palabras como los humanos]{.alert}
    - [Falso.]{.alert} Procesan patrones estadísticos de co-ocurrencia de tokens. No tienen comprensión semántica ni conciencia

- [Si un modelo funciona bien en el conjunto de entrenamiento, funcionará bien en datos nuevos]{.alert}
    - [Falso.]{.alert} Esto describe el sobreajuste (overfitting). El modelo puede memorizar el training set sin generalizar
:::
:::
...
- [El código postal puede ser una variable discriminatoria aunque no incluya raza]{.alert}
    - [Verdadero.]{.alert} El código postal actúa como proxy de raza debido a la segregación residencial histórica. Los sesgos persisten a través de correlaciones

- [BERT y GPT usan la misma arquitectura transformer pero para tareas diferentes]{.alert}
    - [Verdadero.]{.alert} BERT es encoder (comprensión), GPT es decoder (generación). Ambos usan atención, pero con objetivos distintos

- [La validación cruzada sirve para elegir hiperparámetros sin "contaminar" el test set]{.alert}
    - [Verdadero.]{.alert} Permite evaluar múltiples configuraciones usando solo datos de entrenamiento, reservando el test set para la evaluación final
:::
:::
:::
:::
