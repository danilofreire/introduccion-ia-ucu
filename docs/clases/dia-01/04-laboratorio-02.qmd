...
# Cargar paquetes
# install.packages("tidymodels", "tidyverse", "rpart") # Instalar si es necesario
library(tidymodels)
library(tidyverse)
...
3. Elijan [una]{.alert} de estas preguntas y respóndanla con una [comparación numérica]{.alert} (el gráfico es opcional):
    - Elijan [una]{.alert} de [`gasto_salud`]{.alert}, [`desempleo`]{.alert}, [`inversion_extranjera`]{.alert}, [`indice_gobierno_digital`]{.alert} y comparen su media (o mediana) entre países con `crecimiento_alto = "si"` y `"no"`. ¿La diferencia parece grande?
...
- Una lección: no todo feature engineering mejora el modelo. Cuando el predictor original ya capta la información, agregarle un recorte categórico es redundante. Hay que [medirlo]{.alert}, no asumirlo
:::
...
pred_test |> precision(truth = crecimiento_alto, .pred_class, event_level = "second")
pred_test |> recall(truth = crecimiento_alto, .pred_class, event_level = "second")
...
- El prefijo `.` es la convención de tidymodels para columnas que el paquete crea por ti
...
La interacción `indice_gobierno_digital:acceso_internet` mejora un poco el ROC-AUC de [ambos modelos]{.alert}: capta el efecto conjunto de dos variables que se refuerzan, algo que las variables por separado no expresan (la mejora es chica y está dentro del error estándar, así que no conviene sobre-interpretarla). El contraste con `internet_grupos`, que [no ayudó]{.alert}, deja la moraleja: discretizar una variable que ya está en el modelo suele ser redundante, mientras que una [interacción]{.alert} o una transformación no lineal sí puede agregar información nueva
