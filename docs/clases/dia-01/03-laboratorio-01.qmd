...
[Recordatorio:]{.alert} `accuracy` = de todas las predicciones, cuántas acertó; `precision` (también aparece como `ppv`) = de las que predijo "sí", cuántas eran "sí"; `recall` (también `sens`) = de los "sí" reales, cuántos detectó. [¿Cuál de las tres les parece más relevante para este problema?]{.alert} 🤓
:::
