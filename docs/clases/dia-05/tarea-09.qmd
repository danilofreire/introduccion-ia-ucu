...
3. Necesitan [Ollama](https://ollama.com/) corriendo y el modelo del laboratorio: `ollama pull frob/qwen3.5-instruct:4b` (~3,4 GB). [No hace falta ninguna API key]{.alert}: todo corre en su computadora
...
MODELO_LOCAL <- "ollama/frob/qwen3.5-instruct:4b"
```
...
Diseñá desde cero un codebook para clasificar el [motivo principal]{.alert} por el que cada persona no consultó, con las cinco categorías del CSV: `costo`, `estigma`, `desconfianza`, `falta_tiempo` y `autosuficiencia`. Definí cada categoría en las `instructions` e incluí al menos [dos reglas de decisión]{.alert} para casos límite (por ejemplo: ¿qué hacer cuando un relato menciona la plata y la falta de tiempo a la vez?). Nombrá el campo del schema `motivo`.
...
Antes de mirar al LLM: codificá vos. Elegí seis respuestas del corpus, leelas y asignales una categoría [sin mirar la columna]{.alert} `motivo_humano`. Recién después compará tus códigos con los del equipo: ¿en cuántas coinciden? ¿Dónde no, y por qué? El desacuerdo entre codificadores humanos también existe, y conviene conocerlo antes de exigirle perfección a un modelo.

*Pista:* guardá tus códigos en un vector y compará con `consultas$motivo_humano` en esas filas; `mean(tus_codigos == ...)` da la proporción de acuerdo.
...
Codificá las 18 respuestas con el modelo local y validá contra el gold del equipo (`motivo_humano`) a nivel nominal. Reportá la accuracy y el kappa. Con la referencia del curso (kappa ≥ 0,7), ¿confiarían en este codificador?
