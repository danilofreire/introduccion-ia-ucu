...
Miren la diferencia: el LLM tiende a ser [más conservador]{.alert}, sus puntajes se acercan al centro (0). Capta bien el [orden]{.alert} (quién es más iliberal que quién), pero comprime la [magnitud]{.alert}. Esto es justo lo que [la validación va a cuantificar]{.alert}. Los `NA` son llamadas que fallaron contra el modelo `:free` (el callout anterior); en un estudio real se re-codifican esos casos
:::
