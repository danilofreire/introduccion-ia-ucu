...
# Mover `pais` a rownames, luego escalar (media = 0, sd = 1)
datos_scaled <- paises |>
...
[`column_to_rownames("pais")`]{.alert} mueve la columna `pais` a los nombres de fila. Así `scale()` puede trabajar solo con las 8 variables numéricas, y los nombres de países se preservan automáticamente en todos los plots posteriores (clusters, dendrograma, biplot)
:::
