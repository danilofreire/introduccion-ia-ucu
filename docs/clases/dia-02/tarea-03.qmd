...
*Pista (XGBoost):* la sintaxis está en el [Apéndice 4 del Laboratorio 3](07-laboratorio-03.html#sec:appendix04): `boost_tree(trees = 500, tree_depth = tune(), learn_rate = tune()) |> set_engine("xgboost") |> set_mode("classification")`.

*Pista (métricas):* para cada modelo, generen predicciones con `predict()` y `predict(type = "prob")`, luego calculen las métricas con `metrics()` y `f_meas()`.
