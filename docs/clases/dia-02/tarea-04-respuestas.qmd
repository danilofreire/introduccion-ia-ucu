...
```{r p4}
set.seed(2026)
division <- initial_split(datos, prop = 0.8)
...
Definan LASSO con `penalty = tune()`, grilla de 40 lambdas entre 10^-3 y 10^0.5, y CV con 5 folds. ¿Cuál es el mejor lambda?
...
**Respuesta:** El lambda 1SE (~3,2) es casi **3 veces más grande** que el lambda mínimo (~1,1). La lógica de la regla 1SE: si varios lambdas rinden estadísticamente igual (dentro de un error estándar del mínimo), conviene elegir el más grande, porque penaliza más y produce un modelo **más simple**, con más coeficientes en cero. Es la versión regularizada del principio de parsimonia: ante igual desempeño, el modelo con menos variables. El costo es aceptar un RMSE nominalmente un poco peor a cambio de un modelo más interpretable y robusto.
...
**Respuesta:** Con el lambda mínimo, LASSO elimina **10 de los 17 predictores**: `computadoras_alumno`, `horas_semanales`, `formacion_docente_pct` y casi todas las dummies de departamento, sector y zona. El caso de `computadoras_alumno` es el más instructivo: correlaciona ~0,29 con el puntaje (Pregunta 2), pero esa correlación existe sólo porque las computadoras van donde va el presupuesto. Una vez que `presupuesto_alumno` está en el modelo, las computadoras no agregan información independiente y LASSO las pone en cero. `horas_semanales` cae por la razón opuesta: nunca tuvo relación con el puntaje. LASSO no distingue entre "redundante" e "irrelevante": descarta lo que no ayuda a predecir, por dos caminos distintos.
...
**Respuesta:** La mejor combinación es `mixture = 1`, es decir, **LASSO puro**. Elastic Net tenía permitido elegir cualquier mezcla entre Ridge (0) y LASSO (1), y la validación cruzada concluyó que la penalización L1 sola es lo mejor para estos datos. Tiene sentido dada la estructura del dataset: hay un grupo claro de predictores irrelevantes o redundantes (los de la Pregunta 9) que conviene eliminar del todo, y pocos predictores fuertes que no necesitan el "reparto" de Ridge. La lección general: Elastic Net es un buen punto de partida porque contiene a los otros dos como casos particulares y deja que los datos decidan.

## Comparación

### Pregunta 12: Tabla comparativa

Comparen OLS, LASSO, Ridge, Elastic Net y Random Forest en el conjunto de prueba.
...
resultados_rf <- tune_grid(
  wf_rf,
  resamples = folds,
  grid = grid_regular(mtry(range = c(2, 10)), min_n(range = c(5, 30)),
                      levels = c(4, 4))
)

ajuste_rf <- finalize_workflow(
  wf_rf,
  select_best(resultados_rf, metric = "rmse")
) |>
  fit(data = datos_train)

evaluar <- function(ajuste, nombre) {
  augment(ajuste, datos_test) |>
    metrics(truth = puntaje_prueba, estimate = .pred) |>
    mutate(modelo = nombre)
}

bind_rows(
  evaluar(ajuste_ols, "OLS"),
  evaluar(ajuste_lasso, "LASSO"),
  evaluar(ajuste_ridge, "Ridge"),
  evaluar(ajuste_en, "Elastic Net"),
  evaluar(ajuste_rf, "Random Forest")
) |>
...
**Respuesta:** La tabla queda apretadísima: el mejor RMSE es el de **OLS** (~26,7), con LASSO, Elastic Net y Ridge a apenas 0,3-0,5 puntos, y Random Forest levemente detrás (~28). En términos prácticos, **la diferencia es irrelevante**: medio punto de RMSE, en una escala donde el error ronda los 27 puntos, no cambia ninguna decisión. ¿Por qué no gana nadie? Porque con 320 escuelas de entrenamiento y sólo 17 predictores (n mucho mayor que p) y relaciones aproximadamente lineales, OLS no sufre el sobreajuste que la regularización viene a curar, y Random Forest no encuentra no-linealidades que explotar. La regularización brilla con muchos predictores, pocos datos o fuerte colinealidad; acá su valor no es predecir mejor sino producir un modelo más simple (LASSO rinde igual que OLS usando 7 variables en vez de 17).
