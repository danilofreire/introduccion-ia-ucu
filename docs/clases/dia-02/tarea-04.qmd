...
Creen un boxplot que muestre la distribución de `satisfaccion_vida` separada por `zona` (urbana/rural). Agreguen `labs()` con título y etiquetas claras. ¿Hay diferencia entre zonas?
...
*Pista:* usen `select_by_one_std_err()` sobre los resultados del tuning. Necesita la métrica (`"rmse"`) y la dirección en la que ordenar el `penalty`: piensen si quieren el lambda más grande o más pequeño dentro de un error estándar del mínimo.
