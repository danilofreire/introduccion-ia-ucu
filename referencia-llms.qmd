...
En el curso usamos dos variantes del mismo modelo, y la elección importa:

- `qwen3.5:4b`: la variante con razonamiento. Buena para chatear y para problemas difíciles, pero "piensa" uno o dos minutos por cada texto
- `frob/qwen3.5-instruct:4b`: la variante *instruct*, sin razonamiento. Es la que usamos para codificar con `quallmer`: rápida y estable con salida estructurada

```{r ollama}
# En la terminal, una sola vez: ollama pull frob/qwen3.5-instruct:4b
chat <- chat_ollama(model = "frob/qwen3.5-instruct:4b")
...
codificado_local <- qlm_code(
  textos$texto,
  codebook,
  model = "ollama/frob/qwen3.5-instruct:4b",   # CON el prefijo "ollama/"
  max_active = 1,
...
- **El prefijo del modelo**: sin `openrouter/` para `chat_openrouter()`, con `openrouter/` para `qlm_code()`. Lo mismo con Ollama: `chat_ollama(model = "frob/...")` pero `qlm_code(model = "ollama/frob/...")`.
